...
export class InsteonLocalAccessory {
	...
	constructor(
		...
		if (typeof this.device.groupMembers !== 'undefined') {
			this.device.groupMembers = this.device.groupMembers.replace(
				/\s*,\s*/g,
				','
		...
		switch (this.deviceType) {
			...
			case 'garage':
				...
				this.iolinc.on('sensorOn', () => {
					this.log.debug(
						this.name + ' sensor is on. invert_sensor = ' + this.invert_sensor
					)
				...
				this.iolinc.on('sensorOff', () => {
					this.log.debug(
						this.name + ' sensor is off invert_sensor = ' + this.invert_sensor
					)
			...
			case 'smoke':
				...
				this.service =
					this.accessory.getService(
						this.platform.Service.CarbonMonoxideSensor
					) ||
	...
	async setBrightnessLevel(level: CharacteristicValue) {
		...
		if (!this.override && level == this.currentState) {
			...
			this.log.debug(
				'Discard on for ' + this.name + ' already at commanded state'
			)
			return
					...
					this.light.turnOn().then((status) => {
						setTimeout(() => {
							if (status.success) {
								...
								this.log(
									'Error setting power state of ' + this.name + ' to ' + powerOn
								)
								this.getStatus.call(this)
							}
						}, 0)
	...
	setTargetKeypadBtn(callback?) {
		...
		const getButtonMap = (callback) => {
			...
			this.log.debug(
				' Reading button map for target keypad [' +
					this.targetKeypadID[index1] +
					']'
			)

			this.hub.directCommand(
				this.targetKeypadID[index1],
				command,
				timeout,
				(err, status) => {
					if (
						...
						this.log(
							'Error getting button states for target keypad [' +
								this.targetKeypadID[index1] +
								']'
						)
		...
		getButtonMap(() => {
			...
			const binaryButtonMap =
				currentButtonMap.substring(0, buttonNumber) +
			...
			this.hub.directCommand(
				...
				(err, status) => {
					...
					if (
						...
						this.log(
							'Error setting button state of target keypad [' +
								this.targetKeypadID[index1] +
								']'
						)
						this.log.debug('Err: ' + util.inspect(err))
	...
	getGroupMemberStatus() {
		...
		this.groupMembers.forEach((deviceID) => {
			if (/^[0-9a-fA-F]{6}$/.test(deviceID)) {
				...
				if (isDefined) {
					const groupDevice = this.platform.insteonAccessories.filter(
						(item) => {
