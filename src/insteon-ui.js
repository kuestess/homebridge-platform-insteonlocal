...
InsteonUI.prototype.renderHubPage = function (res) {
  ...
  function buildSceneTable(res, callback) {
    ...
    self.scenes.forEach((scene) => {
      var sceneIndex = self.devices.findIndex((item) => {
      ...
      scene.controllers.forEach((controller) => {
        var insteonDevIndex = self.hubDevices.findIndex((item) => {
...
InsteonUI.prototype.renderDevicePage = function (res, deviceID) {
  ...
  if (deviceID == '' || deviceID == null || typeof deviceID === 'undefined') { //nothing selected
    ...
    var devIndex = self.insteonJSON.devices.findIndex((item) => {
  ...
  function buildDeviceInfoTable() {
    ...
    if (typeof device.info === 'undefined' || device.info.length == 0) {
      ...
      var filtered = self.deviceDatabase.filter((item) => {
        return item.category == device.info.deviceCategory.id;
      });
      var found = filtered.find((item) => {
  ...
  function buildOpFlagsTable() {
    if (typeof device.operatingFlags === 'undefined') {
      ...
      if(typeof self.hubDevices[devIndex].info !== 'undefined'){
        var filtered = self.deviceDatabase.filter((item) => {
          return item.category == self.hubDevices[devIndex].info.deviceCategory.id;
        });
        var found = filtered.find((item) => {
          return (item.subcategory == self.hubDevices[devIndex].info.deviceSubcategory.id);
        });
  ...
  if (typeof self.hubDevices === 'undefined' || self.hubDevices.length == 0 || Object.keys(self.insteonJSON.devices).length == 0) {
    ...
    self.hubDevices.forEach((hubDevice) => {
      var linkText;
      if (hubDevice.name == '') {
  ...
  function buildDeviceLinkTable() {
    ...
    self.insteonJSON.devices[devIndex].links.forEach((link) => {
      if (link.isInUse) {
        var insteonDevIndex = self.hubDevices.findIndex((item) => {
          return item.deviceID == link.id;
        });
  ...
  function buildDeviceSceneData(callback) {
    ...
    var deviceGroups = groups.filter((item, index, inputArray) => {
    ...
    deviceGroups.forEach((groupNum) => {
      ...
      var groupLinkArray = self.insteonJSON.devices[devIndex].links.filter((link) => {
      ...
      groupLinkArray.forEach((link) => {
        if (link.controller == true) {
          ...
          var responderIndex = self.hubDevices.findIndex((item) => {
          ...
          var responderLink = self.insteonJSON.devices[responderIndex].links.filter((link) => {
          ...
          if (controllerID == self.hubInfo.id) {
            var hubhubResponderLinks = self.insteonJSON.hub.links.filter((link) => {
            ...
            var controllerIndex = self.hubDevices.findIndex((item) => {
            ...
            var responderLinkArray = [] = self.insteonJSON.devices[controllerIndex].links.filter((responderLink) => {
              return (responderLink.group == groupNum && responderLink.isInUse == true && responderLink.controller == true);
            });

            responderLinkArray.forEach((responderLink) => { //for each of the devices controlled by the controller, get link information
              ...
              if (responderLink.id == self.hubInfo.id) {
                ...
                var responderDevLink = [] = self.insteonJSON.devices[responderIndex].links.filter((link) => {
  ...
  function _getRampRate(deviceID) {
    var linkDevIndex = self.hubDevices.findIndex((item) => {
      return item.deviceID == deviceID;
    });
  ...
  function _getOnLevel(deviceID, controllerID, group) {
    var deviceIndex = self.hubDevices.findIndex((item) => {
      return item.deviceID == deviceID;
    });

    if (deviceIndex != -1 && typeof self.insteonJSON.devices[deviceIndex].links !== 'undefined') {
      var responderLink = self.insteonJSON.devices[deviceIndex].links.filter((link) => {
  ...
  function buildDeviceSceneTable() {
    ...
    self.insteonJSON.devices[devIndex].scenes.forEach((scene) => {
      ...
      scene.controllers.forEach((controller) => {
        ...
        if (insteonDevIndex == -1 || self.hubDevices[insteonDevIndex].info == undefined) {
          ...
          var filtered = self.deviceDatabase.filter((item) => {
            return item.category == self.hubDevices[insteonDevIndex].info.deviceCategory.id;
          });
          var found = filtered.find((item) => {
            return (item.subcategory == self.hubDevices[insteonDevIndex].info.deviceSubcategory.id);
          });
          var groupText;
      ...
      scene.responders.forEach((responder) => {
        ...
        if (insteonDevIndex == -1 || self.hubDevices[insteonDevIndex].info == undefined) {
          ...
          if (found.name.indexOf('Keypad') > -1) {
            var responderLinkData = self.hubDevices[insteonDevIndex].links.filter((link) => {
...
InsteonUI.prototype.renderLinkPage = function (res) {
  ...
  function buildDeviceList() {
    ...
    self.insteonJSON.devices.forEach((device) => {
      if (device.name == '') {
        var listText = device.deviceID;
      } else {
        var listText = device.name;
      }
...
InsteonUI.prototype.handleRequest = function (req, res) {
  ...
  switch (req.url) {
      ...
      if (req.method == 'POST') {
        req.on('data', (chunk) => {
          ...
          function _linkToHub(devices) {
            ...
            self.linkToHub(device, res, (error, response)=> {
              if(error){
      ...
      if (req.method == 'POST') {
        req.on('data', (data) => {
          ...
          function _createScene(scenes) {
            ...
            self.createScene(scene.controller, scene.responder, scene.options, (error, response)=> {
              if(error){
    ...
    case '/getAllDeviceLinks':
      ...
      self.hubDevices.forEach((device) => {
    ...
    case '/getDeviceInfo':
      ...
      self.getAllDeviceInfo(deviceID, res, ()=> {
      ...
      if (req.method == 'POST') {
        req.on('data', (chunk) => {
          ...
          devJSON.forEach((savedDev) => {
            ...
            var devIndex = self.config.platforms[self.platformIndex].devices.findIndex((item) => {
    ...
    default:
      ...
      if (url.indexOf('/removeHubLink') !== -1) {
        ...
        var linkToDelete = self.hubLinks.filter((link) => {
          return link.number == linkNumber;
        });

        linkToDelete = linkToDelete[0];

        self.removeLink(deviceID, linkToDelete, (error, response)=> {
          if(error){
      ...
      if (url.indexOf('/getLinks') !== -1) {
        ...
        self.selectedDevice = deviceID;
        self.getDeviceLinks(deviceID, (error, links) => {
          if (error) {
            res.write(self.header + self.navBar);
...
InsteonUI.prototype.getDeviceInfo = function (deviceID, res, callback) {
  ...
  self.hub.info(deviceID, (error, info) => {
    if (error || info == null) {
...
InsteonUI.prototype.getDatabaseDelta = function (deviceID, callback) {
  ...
  self.hub.directCommand(deviceID, cmd, timeout, (error, response) => {
...
InsteonUI.prototype.saveConfig = function (res, backup) {
  ...
  fs.copyFile(self.configPath, self.configPath + '.bak', (err) => {
    if (err) {
      console.log('Error creating backup: ' + err);
    } else {
      console.log('Created backup of previous config');
    }
  });
  ...
  fs.writeFile(self.configPath, newConfig, 'utf8', (err, data) => {
    if (err) {
...
InsteonUI.prototype.saveInsteonConfig = function (res) {
  ...
  //make backup of old config
  fs.copyFile(self.configDir + './insteon.json', self.configDir + './insteon.json.bak', (err) => {
    if (err) {
  ...
  res.end(self.footer);
  fs.writeFile(self.configDir + './insteon.json', newInsteonJSON, 'utf8', (err, data) => {
    if (err) {
      return console.log(err);
    } else {
...
InsteonUI.prototype.loadConfig = function () {
  ...
  self.platformIndex = self.config.platforms.findIndex((item) => {
    return item.platform == 'InsteonLocal';
  });

  var platform = self.platforms.filter((item) => {
    return item.platform == 'InsteonLocal';
  });
...
InsteonUI.prototype.getHubDevices = function (res, callback) {
  ...
  self.hub.links((error, links) => {
    if (error) {
      ...
      var hubDevices = devices.filter((item, index, inputArray) => {
        return inputArray.indexOf(item) == index;
      });

      hubDevices.forEach((deviceID) => {
        var devName;
        var configDevIndex = self.config.platforms[self.platformIndex].devices.findIndex((item) => {
        ...
        if (typeof oldDevices === 'undefined' || Object.keys(oldDevices).length == 0) {
          ...
          var insteonDevIndex = oldDevices.findIndex((item) => {
            return item.deviceID == deviceID;
          });
        }
...
InsteonUI.prototype.getAllDeviceInfo = function (deviceID, res, callback) {
  ...
  self.getDeviceInfo(deviceID, res, (error, response) => {
    if (error) {
      sse.emit('push', { 'message': 'Error getting device info for ' + deviceID });
    }

    self.getOpFlags(deviceID, (error, response) => {
      if (error) {
        sse.emit('push', { 'message': 'Error getting operating flags for ' + deviceID });
      }

      self.getDatabaseDelta(deviceID, (error, dbdelta) => {
        ...
        if (recentDelta == databaseDelta) {
          ...
          self.getDeviceLinks(deviceID, (error, response) => {
            if (error) {
...
InsteonUI.prototype.getDeviceLinks = function (deviceID, callback) {
  ...
  function _getDeviceLinks(deviceID, at, linkArray, callback) {
    self.hub.linkAt(deviceID, at, linkArray, (error, links) => {
      if (error) {
...
InsteonUI.prototype.linkToHub = function (deviceID, res, callback) {
  ...
  self.hub.link(deviceID, options, (error, link) => {
    if (error || link == null) {
...
InsteonUI.prototype.unlinkFromHub = function (deviceID, res) {
  ...
  self.hub.unlink(deviceID, (error, link) => {
    if (error) {
...
InsteonUI.prototype.removeLinkAt = function (deviceID, linkAt, res) {
  ...
  var link = self.insteonJSON.devices[devIndex].links.filter((item) => {
  ...
  self.hub.modLink('remove', deviceID, link, (error, response) => {
    if (error) {
      console.log('Error removing link from device');
      return;
    } else {
      console.log('Link: ' + util.inspect(response));
      var linkIndex = self.insteonJSON.devices[devIndex].links.findIndex((item) => {
        return item.at == linkAt;
      });
...
InsteonUI.prototype.removeLink = function (deviceID, link, callback) {
  ...
  if (deviceID == null || deviceID == self.hubInfo.id) {
    ...
    self.hub.sendCommand(command, timeout, (error, response) => {
      if (error || response.success == false) {
        ...
        var linkIndex = self.insteonJSON.hub.links.filter((link)=> {
          return link.number == linkNumber;
        });
...
InsteonUI.prototype.createScene = function (controller, responder, options, callback) {
  ...
  self.hub.scene(controller, responder, options, (error, link) => {
    if (error) {
...
InsteonUI.prototype.buildHubSceneData = function (callback) {
  ...
  //filter for unique groups
  var hubGroups = groups.filter((item, index, inputArray) => {
    return inputArray.indexOf(item) == index;
  });
