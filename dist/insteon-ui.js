...
exports.InsteonUI = void 0;
const deviceDatabase_json_1 = __importDefault(require("./deviceDatabase.json"));
const utils_1 = __importDefault(require("home-controller/lib/Insteon/utils"));
const chalk_1 = __importDefault(require("chalk"));
const moment_1 = __importDefault(require("moment"));
const byteToRampRate = utils_1.default.byteToRampRate;
const toByte = utils_1.default.toByte;
let rampRates = utils_1.default.RAMP_RATES;
const util_1 = __importDefault(require("util"));
const fs_1 = __importDefault(require("fs"));
const events_1 = __importDefault(require("events"));
const sse = new events_1.default();
let ui;
class InsteonUI {
    constructor(configPath, hub) {
        ...
        this.deviceDatabase = deviceDatabase_json_1.default;
    ...
    renderMainPage(res) {
        ...
        const buildHubTable = () => {
            this.hubTable = '';
            const hubUsername = '<div class=\'form-group\'><label for=\'username\'>Username:</label><input type=\'text\' class=\'form-control\' name=\'hubUsername\' value=\'' +
                this.platform.user +
                '\'></div>';
            const hubPassword = '<div class=\'form-group\'><label for=\'password\'>Password:</label><input type=\'password\' class=\'form-control\' name=\'hubPassword\' value=\'' +
                this.platform.pass +
                '\'></div>';
            const hubAddress = '<div class=\'form-group\'><label for=\'host\'>Hub address:</label><input type=\'text\' class=\'form-control\' name=\'hubAddress\' value=\'' +
                this.platform.host +
                '\'></div>';
            const hubPort = '<div class=\'form-group\'><label for=\'port\'>Hub port:</label><input type=\'text\' class=\'form-control\' name=\'hubPort\' value=\'' +
                this.platform.port +
                '\'></div>';
            const hubModel = '<div class=\'form-group\'><label for=\'model\'>Hub model:</label><input type=\'text\' class=\'form-control\' name=\'hubModel\' value=\'' +
                this.platform.model +
                '\'></div>';
            const hubRefresh = '<div class=\'form-group\'><label for=\'refresh\'>Refresh:</label><input type=\'text\' class=\'form-control\' name=\'hubRefresh\' value=\'' +
                this.platform.refresh +
                '\'></div>';
            const hubKeepalive = '<div class=\'form-group\'><label for=\'keepalive\'>Keepalive:</label><input type=\'text\' class=\'form-control\' name=\'hubKeepalive\' value=\'' +
    ...
    renderHubPage(res) {
        ...
        if (typeof this.hubLinks === 'undefined' ||
        ...
        if (this.hubInfo == undefined || this.hubInfo.length == 0) {
            ...
            this.hubInfoTable = '';
            const hubAddress = '<div class=\'form-group\'><b>Hub address: </b>' +
                this.platform.host +
                '</div>';
            const hubPort = '<div class=\'form-group\'><b>Hub port: </b>' +
                this.platform.port +
                '</div>';
            const hubModel = '<div class=\'form-group\'><b>Hub model: </b>' +
                this.platform.model +
                '</div>';
            const hubID = '<div class=\'form-group\'><b>ID: </b>' + this.hubInfo.id + '</div>';
            const hubFirmwareVersion = '<div class=\'form-group\'><b>Firmware Version: </b>' +
        ...
        const buildSceneTable = (res, callback) => {
            ...
            this.scenes.forEach((scene) => {
                ...
                let controllerTable = '<h4>Controllers - ' +
                    sceneNameText +
                    '</h4>' +
                    this.sceneControllerTableHeader +
                    '<tbody>';
                let responderTable = '<h4>Responders - ' +
                    sceneNameText +
                    '</h4>' +
    ...
    renderDevicePage(res, deviceID) {
        const _getRampRate = (deviceID) => {
            ...
            else if (typeof this.insteonJSON.devices[linkDevIndex].operatingFlags !==
                'undefined' &&
                typeof this.insteonJSON.devices[linkDevIndex].operatingFlags.D7 !==
                    'undefined') {
                rampVal = byteToRampRate(this.insteonJSON.devices[linkDevIndex].operatingFlags.D7);
            }
            else {
                rampVal = '';
            }
        ...
        const _getOnLevel = (deviceID, controllerID, group) => {
            ...
            if (deviceIndex != -1 &&
                ...
                const responderLink = this.insteonJSON.devices[deviceIndex].links.filter((link) => {
        ...
        const buildDeviceSceneTable = () => {
            ...
            if (typeof this.insteonJSON.devices[devIndex].scenes === 'undefined' ||
                this.insteonJSON.devices[devIndex].scenes.length == 0) {
                this.deviceSceneTable =
                    '<p>No scenes defined for ' +
                        this.insteonJSON.devices[devIndex].name +
                        '</p>';
                this.log('No scenes defined for ' + this.insteonJSON.devices[devIndex].name);
                return;
            }
            this.insteonJSON.devices[devIndex].scenes.forEach((scene) => {
                let controllerTable = '<h4>Controllers - Scene ' +
                    scene.group +
                    '</h4>' +
                    this.deviceSceneControllerTableHeader +
                    '<tbody>';
                let responderTable = '<h4>Responders - Scene ' +
                    scene.group +
                    '</h4>' +
                ...
                scene.responders.forEach((responder) => {
                    ...
                    if (insteonDevIndex == -1 ||
                        ...
                        if (found.name.indexOf('Keypad') > -1) {
                            const responderLinkData = this.hubDevices[insteonDevIndex].links.filter((link) => {
                                return link.group == responder.group;
                            });
                            if (responderLinkData.length > 0 &&
        ...
        if (deviceID == '' || deviceID == null || typeof deviceID === 'undefined') {
            ...
            const buildOpFlagsTable = () => {
                if (typeof device.operatingFlags === 'undefined') {
                    ...
                    const onLevel = Math.ceil((parseInt(device.operatingFlags.D8, 16) / 255) * 100);
                    const ledBrightness = Math.ceil((parseInt(device.operatingFlags.D9, 16) / 128) * 100);
                    const databaseDelta = typeof device.databaseDelta === 'undefined'
                        ? ''
                        : device.databaseDelta;
                    const progLock = typeof device.operatingFlags.programLock === 'undefined'
                        ? ''
                        : device.operatingFlags.programLock;
                    const resumeDim = typeof device.operatingFlags.resumeDim === 'undefined'
                        ? ''
                        : device.operatingFlags.resumeDim;
                    const ledEnable = typeof device.operatingFlags.ledEnable === 'undefined'
                        ? ''
                        : device.operatingFlags.ledEnable;
                    const keys = typeof device.operatingFlags.keys === 'undefined'
                        ? ''
                        : device.operatingFlags.keys;
                    let keyRow;
                    if (typeof this.hubDevices[devIndex].info !== 'undefined') {
                        const filtered = this.deviceDatabase.filter((item) => {
                            return (item.category == this.hubDevices[devIndex].info.deviceCategory.id);
                        });
                        const found = filtered.find((item) => {
                            return (item.subcategory ==
            ...
            const buildDeviceSceneData = (callback) => {
                ...
                const scenes = [];
                const responders = [];
                this.log('Building scene data for ' +
                    this.insteonJSON.devices[devIndex].name +
                    '...');
                this.insteonJSON.devices[devIndex].links.forEach((link) => {
                ...
                deviceGroups.forEach((groupNum) => {
                    ...
                    let groupLinkArray = this.insteonJSON.devices[devIndex].links.filter((link) => {
                    ...
                    groupLinkArray.forEach((link) => {
                        if (link.controller == true) {
                            ...
                            let responderLink = this.insteonJSON.devices[responderIndex].links.filter((link) => {
                            ...
                            if (typeof responderLink !== 'undefined' &&
                            ...
                            if (controllerID == this.hubInfo.id) {
                                ...
                                if (controllerIndex == -1 ||
                                    typeof this.insteonJSON.devices[controllerIndex].links ===
                                        'undefined') {
                                    //device not in insteon.json
                                    this.log('Device/links for ' + controllerID + ' not found in config');
                                    return;
                                }
                                const responderLinkArray = ([] = this.insteonJSON.devices[controllerIndex].links.filter((responderLink) => {
                                    return (responderLink.group == groupNum &&
                                    ...
                                    if (responderLink.id == this.hubInfo.id) {
                                        ...
                                        if (responderIndex == -1 ||
                                            typeof this.insteonJSON.devices[responderIndex].links ===
                                                'undefined') {
                                            //device not in insteon.json
                                            this.log('Device/links for ' +
                                                responderLink.id +
                                                ' not found in config');
                                            return;
                                        }
                                        const responderDevLink = ([] = this.insteonJSON.devices[responderIndex].links.filter((link) => {
                                            return (link.group == groupNum &&
                                                link.isInUse == true &&
                                                link.controller == false);
                                        }));
            ...
            const buildDeviceInfoTable = () => {
                ...
                if (typeof device.info === 'undefined' || device.info.length == 0) {
                    ...
                    const deviceIDRow = '<div class="form-group row"><b>Device ID: </b>' +
                        device.deviceID +
                        '</div>';
                    const firmware = '<div class="form-group row"><b>Firmware: </b>' +
                        device.info.firmware +
                        '</div>';
                    const deviceDim = '<div class="form-group row"><b>Dimmable: </b>' +
                        device.info.isDimmable +
                        '</div>';
                    const deviceCat = '<div class="form-group row"><b>Device Category: </b>' +
                    ...
                    const deviceModel = '<div class="form-group row"><b>Model: </b>' + model + '</div>';
                    const deviceDesc = '<div class="form-group row"><b>Description: </b>' +
                        description +
                        '</div>';
                    this.hubDeviceInfoTable =
                        this.hubDeviceInfoTable +
        ...
        res.write('<a class=\'btn btn-default load center-block getlinks\' onclick=\'sseInit(this)\' id=\'getAllLinks\' role=\'button\' href=\'/getAllDeviceLinks\' style=\'width:135px; height:30px;\'>Get All Dev Links</a>');
        res.write('</div>');
        ...
        if (typeof this.selectedDevice === 'undefined' ||
            ...
            res.write('<h3 class=\'sub-header\'>Device Info - ' +
                devName +
    ...
    renderAddPage(res, type) {
        ...
        res.write('<form enctype=\'application/x-www-form-urlencoded\' action=\'/save' +
            type +
	...
	<div>Create Scene</div>
        ...
        res.write(this.sceneTemplateHeader +
        ...
        switch (req.url) {
            ...
            case '/saveDeviceSettings':
                if (req.method == 'POST') {
                    ...
                    req.on('end', () => {
                        ...
                        const referer = req.header('Referer');
                        const deviceID = referer.substring(referer.lastIndexOf('/') + 1, referer.length);
                    ...
                    req.on('end', () => {
                        ...
                        controllers.forEach((controller) => {
                            for (let i = 0; i < responders.length; i++) {
                                ...
                                if (controller.id.toUpperCase() == this.hubInfo.id.toUpperCase()) {
                                    controller.id = 'gw';
                                }
                                if (responders[i].id.toUpperCase() ==
                        ...
                        const _createScene = (scenes) => {
                            ...
                            sse.emit('push', {
                                message: 'Linking ' + scene.responder.id + ' to ' + scene.controller,
                            });
                            this.createScene(scene.controller, scene.responder, scene.options, (error, response) => {
                                if (error) {
            ...
            case '/getDeviceInfo':
                ...
                if (deviceID == 'devices') {
                    ...
                    this.log('Error getting device id from url: ' +
                        referer +
                ...
                if (req.method == 'POST') {
                    ...
                    req.on('end', () => {
                        ...
                        devJSON.forEach((savedDev) => {
                            if (this.config.platforms[this.platformIndex].devices == undefined) {
                                this.log('No devices defined - adding to config');
                                this.config.platforms[this.platformIndex].devices = [];
                            }
                            const devIndex = this.config.platforms[this.platformIndex].devices.findIndex((item) => {
                            ...
                            if (devIndex == -1) {
                                ...
                                const insertIndex = this.config.platforms[this.platformIndex].devices.length;
                                this.config.platforms[this.platformIndex].devices[insertIndex] =
                                    {};
                                keys.forEach((key) => {
                                    this.config.platforms[this.platformIndex].devices[insertIndex][key] = savedDev[key];
                                });
                            }
                            else {
                                //modify existing
                                ...
                                keys.forEach((key) => {
                                    ...
                                    this.config.platforms[this.platformIndex].devices[devIndex][key] = savedDev[key];
                                });
                            }
                        });
            ...
            case '/addToConfig':
                ...
                this.generateDeviceConfig(this.selectedDevice, res, (error, devConf, res) => {
                    if (error) {
                        this.log('Error - could not generate device config for ' +
                            this.selectedDevice);
                        res.write(this.header + this.navBar);
                        res.write('<div class=\'alert alert-danger alert-dismissible fade in out alert-close\' id=\'saveAlert\'><a href=\'/devices/' +
                            this.selectedDevice +
                        ...
                        this.log('Device config for ' +
                            this.selectedDevice +
                            ' is: ' +
                            util_1.default.inspect(devConf));
            ...
            default:
                ...
                if (url.indexOf('/getLinks') !== -1) {
                    ...
                    this.getDeviceLinks(deviceID, (error, links) => {
                        if (error) {
                            res.write(this.header + this.navBar);
                            res.write('<div class=\'alert alert-danger alert-dismissible fade in out\' id=\'saveAlert\'><a href=\'/devices/' +
                                this.selectedDevice +
    ...
    buildDeviceList() {
        if (typeof this.insteonJSON.devices === 'undefined' ||
            Object.keys(this.insteonJSON.devices).length == 0) {
    ...
    saveConfig(res) {
        ...
        fs_1.default.copyFile(this.configPath, this.configPath + '.bak', (err) => {
            if (err) {
        ...
        fs_1.default.writeFile(this.configPath, newConfig, 'utf8', (err) => {
            if (err) {
                this.log(err);
                return;
            }
    ...
    saveInsteonConfig(res) {
        let newInsteonJSON = this.stripEscapeCodes(JSON.stringify(this.insteonJSON));
        newInsteonJSON = JSON.stringify(JSON.parse(newInsteonJSON), null, 4);
        //make backup of old config
        fs_1.default.copyFile(this.configDir + './insteon.json', this.configDir + './insteon.json.bak', (err) => {
            if (err) {
        ...
        res.write(this.header + this.navBar);
        res.write('<div class=\'alert alert-success alert-dismissible fade in out\' id=\'saveAlert\'><a href=\'/devices/' +
            this.selectedDevice +
    ...
    loadConfig() {
        ...
        const configJSON = fs_1.default.readFileSync(this.configPath);
        ...
        for (let i = 0; i < this.devices.length; i++) {
            if (this.devices[i].deviceID) {
                if (this.devices[i].deviceID.includes('.')) {
                    this.devices[i].deviceID = this.devices[i].deviceID.replace(/\./g, '');
                }
            }
        }
        if (typeof this.config.platforms[this.platformIndex].devices === 'undefined') {
            this.config.platforms[this.platformIndex].devices = [];
        }
    }
    loadInsteonConfig(callback) {
        if (fs_1.default.existsSync(this.configDir + './insteon.json')) {
            this.log('Reading devices from insteon.json');
            const insteonJSON = fs_1.default.readFileSync(this.configDir + './insteon.json');
    ...
    getHubInfo() {
        this.hub.info((error, info) => {
            if (error || typeof info === 'undefined') {
                ...
                this.insteonJSON.hub.info = this.hubInfo;
                this.log('Hub/PLM info is ' + util_1.default.inspect(this.hubInfo));
                return;
            }
        });
    }
        ...
        this.hub.links((error, links) => {
            if (error) {
                ...
                hubDevices.forEach((deviceID) => {
                    let devName;
                    const configDevIndex = this.config.platforms[this.platformIndex].devices.findIndex((item) => {
                        return item.deviceID == deviceID && item.deviceType != 'scene';
                    });
                    let insteonDevIndex;
                    if (typeof oldDevices === 'undefined' ||
    ...
    getOpFlags(deviceID, callback) {
        ...
        this.hub.directCommand(deviceID, cmd, timeout, (error, response) => {
            if (error ||
                ...
                const progLock = parseInt(binaryMap.substring(3, 1)) == 1 ? 'on' : 'off'; //1=on, 0=off
                const ledEnable = parseInt(binaryMap.substring(2, 1)) == 1 ? 'on' : 'off'; //1=LED on, 0=LED off
                const resumeDim = parseInt(binaryMap.substring(1, 1)) == 1 ? 'on' : 'off'; //1=resume dim enabled, 0=resume dim disabled
                const keys = parseInt(binaryMap.substring(0, 1)) == 1 ? 8 : 6; //1=8 keys, 0=6 keys
                if (devIndex !== -1) {
                    if (typeof this.insteonJSON.devices[devIndex].operatingFlags ===
                        'undefined') {
                        this.insteonJSON.devices[devIndex].operatingFlags = {};
                    }
    ...
    removeLinkAt(deviceID, linkAt, res) {
        ...
        this.hub.modLink('remove', deviceID, link, (error, response) => {
            if (error) {
                ...
                this.log('Link: ' + util_1.default.inspect(response));
                const linkIndex = this.insteonJSON.devices[devIndex].links.findIndex((item) => {
                    return item.at == linkAt;
                });
                this.log('Successfully removed link at ' + linkAt + ' for ' + deviceID);
    ...
    buildHubSceneData(callback) {
        ...
        hubGroups.forEach((groupNum) => {
            ...
            const controllers = [];
            let controllerID;
            if (typeof this.hubInfo === 'undefined' ||
...
//# sourceMappingURL=insteon-ui.js.map