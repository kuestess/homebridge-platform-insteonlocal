...
exports.InsteonLocalAccessory = void 0;
const moment_1 = __importDefault(require("moment"));
const util_1 = __importDefault(require("util"));
const underscore_1 = __importDefault(require("underscore"));
const utils_1 = __importDefault(require("home-controller/lib/Insteon/utils"));
const convertTemp = utils_1.default.convertTemp;
class InsteonLocalAccessory {
    constructor(platform, accessory) {
        ...
        this.override = this.platform.override || false;
        ...
        if (this.deviceType == 'keypad') {
            this.keypadbtn =
                typeof this.device.keypadbtn === 'string' ? this.device.keypadbtn : '?';
    ...
    init() {
        ...
        switch (this.deviceType) {
            ...
            case 'scene':
                ...
                this.service
                    .getCharacteristic(this.platform.Characteristic.On)
                    .onSet(this.setSceneState.bind(this));
                this.hub.once('connect', () => {
                    if (this.id) {
            ...
            case 'garage':
                ...
                this.service
                    ...
                    .onSet(this.setRelayState.bind(this));
                ...
                this.iolinc.on('sensorOn', () => {
                    ...
                    if (this.invert_sensor == false || this.invert_sensor == 'false') {
                        //Door Closed (non-inverted): No delay to action, since sensor isn't triggered until door is fully closed.
                        this.log.debug(' >>> No Delayed Action <<<');
                        actionDoorClosed();
                    }
                    else {
                        //Door Open (inverted): Add delay to action, since sensor is triggered immediately upon door closing.
                        setTimeout(() => {
                ...
                this.iolinc.on('sensorOff', () => {
                    ...
                    if (this.invert_sensor == false || this.invert_sensor == 'false') {
                        //Door Open (non-inverted): Add delay to action, since sensor is triggered immediately upon door opening.
                        setTimeout(() => {
                            this.log.debug(' >>> Delayed Action <<<');
                            actionDoorOpen();
                        }, 1000 * this.gdo_delay);
                        ...
                        //Door Closed (inverted): No delay to action, since sensor isn't triggered until door fully opens.
                        this.log.debug(' >>> No Delayed Action <<<');
                        actionDoorClosed();
                    }
                });
                this.hub.once('connect', () => {
                    this.getSensorStatus.call(this, () => {
                        /*do nothing for now*/
                    });
                });
            ...
            case 'leaksensor': {
                ...
                this.service
                    ...
                    .updateValue(0); //Initialize as dry
                this.service
                ...
                if (this.disableBatteryStatus == false) {
                    ...
                    this.heartbeatTimer = setTimeout(() => {
                        ...
                        this.service
                            .getCharacteristic(this.platform.Characteristic.StatusLowBattery)
                            .updateValue(1); //0=normal, 1=low
                    }, 1000 * 24 * 60 * 60 + buffer);
                }
                if (this.disableBatteryStatus == true) {
                    //if battery status disabled, still notify when the device is dead
            ...
            case 'motionsensor': {
                ...
                this.service
                    ...
                    .updateValue(0); //Initialize with no motion
                this.service
                    .getCharacteristic(this.platform.Characteristic.StatusLowBattery)
                    .updateValue(0); //0=normal, 1=low
                this.statusLowBattery = false;
            ...
            case 'contactsensor': {
                ...
                this.service
                    ...
                    .updateValue(0); //Initialize closed
                this.service
                    .getCharacteristic(this.platform.Characteristic.StatusLowBattery)
                    .updateValue(0); //0=normal, 1=low
                this.statusLowBattery = false;
                ...
                this.door.on('opened', () => {
                    ...
                    else if (this.invert_sensor == true ||
                        this.invert_sensor == 'true') {
                        this.currentState = false;
                        this.service
                            .getCharacteristic(this.platform.Characteristic.ContactSensorState)
                            .updateValue(0);
            ...
            case 'remote':
                ...
                this.handleRemoteEvent = (gateway, command1) => {
                    const group = parseInt(gateway.substring(4, 6));
                    const configuredBtn = parseInt(this.button);
                    if (group !== configuredBtn) {
                        this.log.debug('Remote event for button ' + group + ', not configured button ' + configuredBtn);
                        return;
                    }
            ...
            case 'outlet':
                ...
                this.service
                    .getCharacteristic(this.platform.Characteristic.On)
                    .onSet(this.setOutletState.bind(this));
                this.dimmable = false;
                this.hub.once('connect', () => {
            ...
            case 'keypad':
                ...
                this.service
                    .getCharacteristic(this.platform.Characteristic.On)
                    .onSet(this.setKeypadState.bind(this));
                this.hub.once('connect', () => {
                    this.getSceneState.call(this);
                });
                break;
            ...
            case 'x10':
                ...
                this.service
                    .getCharacteristic(this.platform.Characteristic.On)
                    .onSet(this.setX10PowerState.bind(this));
    ...
    async setBrightnessLevel(level) {
        if (this.disabled) {
        ...
        this.log.debug('Command for ' +
            ...
            ', delta: ' +
            delta);
        if (!this.override && level == this.currentState) {
            //allow Insteon command if override is true
        ...
        const setLevel = (level) => {
            ...
            this.hub.directCommand(this.id, cmd, timeout, (error, status) => {
                ...
                if (this.targetKeypadID.length > 0) {
                    this.log.debug(this.targetKeypadID.length +
                        ' target keypad(s) found for ' +
                        this.name);
                    for (let temp = 0; temp < this.targetKeypadID.length; temp++) {
                        this.log.debug(' targetKeypadID[' +
                            temp +
                            '] = ' +
                            '[' +
                            this.targetKeypadID[temp] +
                            ']');
                    }
                    let count;
    ...
    getSceneState() {
        ...
        this.hub.directCommand(this.id, cmd, timeout, (err, status) => {
            if (err ||
                ...
                typeof status.response === 'undefined' ||
                typeof status.response.standard === 'undefined' ||
                status.success == false) {
                this.log('Error getting power state of ' + this.name);
            }
            else {
                if (this.keypadbtn == 'ON' ||
                    (this.six_btn == false && this.keypadbtn == 'A') ||
                    (typeof this.six_btn === 'undefined' && this.keypadbtn == 'A')) {
                ...
                binaryButtonMap =
                    '00000000'.substr(binaryButtonMap.length) + binaryButtonMap; //pad to 8 digits
    ...
    getFanState() {
        ...
        this.light.fan((error, state) => {
            if (error || typeof state === 'undefined') {
                ...
                this.service
                    .getCharacteristic(this.platform.Characteristic.RotationSpeed)
                    .updateValue(this.level); //value from 1 to 100
            }
        });
    }
    getSensorStatus(callback) {
        ...
        this.iolinc.status((error, status) => {
            if (error ||
                status == null ||
                typeof status === 'undefined' ||
                typeof status.sensor === 'undefined') {
                return;
            }
            else {
                this.log.debug('Invert sensor: ' + this.invert_sensor);
    ...
    setPowerState(state) {
        ...
        this.platform.checkHubConnection();
        if (!this.override && state !== this.currentState) {
            //allow Insteon command if override is true
            ...
            if (state) {
                setTimeout(() => {
                    this.light.turnOn().then((status) => {
                        setTimeout(() => {
                            if (status.success) {
                                //if command actually worked, do nothing
                                //do nothing
                            }
                            else {
                                //if command didn't work, check status to see what happened and update homekit
    ...
    setTargetKeypadBtn(callback) {
        ...
        this.log(' also setting target keypad [' +
            this.targetKeypadID[index1] +
            '] button [' +
            this.targetKeypadBtn[index1] +
            '] to ' +
            this.currentState);
        ...
        const getButtonMap = (callback) => {
            ...
            this.log.debug(' Reading button map for target keypad [' +
                this.targetKeypadID[index1] +
                ']');
            this.hub.directCommand(this.targetKeypadID[index1], command, timeout, (err, status) => {
                if (err ||
                    ...
                    typeof status.response.standard === 'undefined' ||
                    status.success == false) {
                    this.log('Error getting button states for target keypad [' +
                        this.targetKeypadID[index1] +
                        ']');
                    this.log.debug('Err: ' + util_1.default.inspect(err));
                    return;
                }
                else {
                    const hexButtonMap = status.response.standard.command2;
        ...
        getButtonMap(() => {
            ...
            const buttonMap = ('00' + parseInt(binaryButtonMap, 2).toString(16))
                .substr(-2)
                .toUpperCase();
            ...
            this.hub.directCommand(this.targetKeypadID[index1], cmd, timeout, (err, status) => {
                ...
                if (err ||
                    ...
                    typeof status.response.standard === 'undefined' ||
                    status.success == false) {
                    this.log('Error setting button state of target keypad [' +
                        this.targetKeypadID[index1] +
                        ']');
                    this.log.debug('Err: ' + util_1.default.inspect(err));
    ...
    getGroupMemberStatus() {
        ...
        this.groupMembers.forEach((deviceID) => {
            if (/^[0-9a-fA-F]{6}$/.test(deviceID)) {
                //group member defined by id
                const isDefined = underscore_1.default.contains(deviceIDs, deviceID.toUpperCase(), 0);
                if (isDefined) {
            ...
            this.iolinc.relayOn().then((status) => {
                if (status.success) {
                    if (this.deviceType == 'iolinc') {
                        ...
                        this.log(' >>> New target state is ' +
                            this.targetState +
                            (this.targetState == 0 ? ' (Open)' : ' (Closed)'));
                        return;
                    }
    ...
    getPosition() {
        //get position for shades/blinds
        this.platform.checkHubConnection();
        this.log('Getting status for ' + this.name);
        this.light.level((err, level) => {
            if (err || level == null || typeof level === 'undefined') {
        ...
        if (state) {
            ...
            this.hub.directCommand(this.id, cmd, timeout, (error, status) => {
                if (error ||
                    ...
                    typeof status.response === 'undefined') {
                    this.log('Error getting power state of ' + this.name);
                    this.log.debug('Error: ' + util_1.default.inspect(error));
                    return;
                }
                else {
                    if (status.success) {
    ...
    getThermostatStatus() {
        this.thermostat.status((err, status) => {
            ...
            this.log.debug('Status: ' + util_1.default.inspect(status));
            //get mode
            if (status.mode == 'off') {
                this.service
                    .getCharacteristic(this.platform.Characteristic.CurrentHeatingCoolingState)
    ...
    async setTemperature(temp) {
        ...
        if (this.mode == 'heat') {
            this.thermostat.heatTemp(theTemp, (err, response) => {
                ...
                this.log.debug(util_1.default.inspect(response));
                this.service
                    .getCharacteristic(this.platform.Characteristic.TargetTemperature)
                    .updateValue(temp);
...
exports.InsteonLocalAccessory = InsteonLocalAccessory;
//# sourceMappingURL=InsteonLocalAccessory.js.map